...
if nargin < 3
  ...
  elseif nobj == 3
    ...
    error('mop:eq:sproblem1', 'The vector a of size (n x nobj) should be specified.');
  end
else
  a = a(:, :);  
  n = size(a, 1);
...
objfun = struct('f', @sproblem1f, 'J', @sproblem1J, 'H', @sproblem1H);
...
opts.MOPName = 'SProblem with lin eq';

% Pareto set and front
pset = [];
pfront = [];

  function [fx] = sproblem1f(x)
