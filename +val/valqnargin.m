...
function [Bx0, x0, Jx0, x1, Jx1, m, n, nobj] = valqnargin(Bx0, x0, Jx0, x1, Jx1, opts)
...
if ~doval
  ...
  if isempty(Bx0)
    if m0 > 1
      Bx0 = utils.eyen([m0 n n nobj], 2, 3); % (m0 x n x n x nobj)
    else
      Bx0 = utils.eyen([n n nobj], 1, 2); % (n x n x nobj)
    end
  end 
  return
end
...
if isempty(x0) && (~isempty(Jx0) || ~isempty(Bx0))
  error('val:valqnargin:EmptyX0', 'The previous iteration point x0 must be specified since a non empty value of Jx0 and Bx0 was specified.')
end
...
if isempty(Jx0) && (~isempty(x0) || ~isempty(Bx0))
...
if isempty(Bx0)
  ...
  if m0 == 1
    [n1Bx0, n2Bx0, nobjBx0] = size(Bx0);
  else
    [mBx0, n1Bx0, n2Bx0, nobjBx0] = size(Bx0);
    if mBx0 ~= m0
      error('val:valqnargin:X0AndBx0InconsistentM', 'The number of individuals is inconsistent in the specified x0 and Bx0.');
    end
  end
  if n1Bx0 ~= n || n2Bx0 ~= n
    error('val:valqnargin:X0AndBx0InconsistentN', 'The number of variables is inconsistent in the specified Bx0.');
  end
  if nobjBx0 ~= nobj
    error('val:valqnargin:Bx0InconsistentNobj', 'The number of objectives is inconsistent in the specified Bx0.');
  end
  if opts.FunValCheck
    val.checkval(Bx0, 'Bx0', false);
  end
end
end